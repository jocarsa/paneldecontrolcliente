    ...
    function cargaTabla(tabla){
        ...
        .then(function(datos){                                      // Cuando el paso anterior haya sido completado
            ...
            claves.forEach(function(clave){                         // Para cada una de las claves
                ...
                camposenviar.push(entrada)
                entrada.setAttribute("placeholder",clave)
                modal.appendChild(entrada)
                
            })
            let columnaoperaciones = document.createElement("th")
            cabecera.appendChild(columnaoperaciones)
            let botonenviar = document.createElement("button")
            botonenviar.textContent = "Enviar"
            botonenviar.onclick = function(){
                console.log("Enviamos los datos")
                let datosenviar = {}
                camposenviar.forEach(function(elemento){
                    datosenviar[elemento.getAttribute("placeholder")] = elemento.value
                })
            
                console.log(datosenviar)
                fetch("nuevoregistro.php?tabla="+globaltabla, {
                  method: 'POST', 
                  headers: {
            ...
            datos.forEach(function(registro){                       // Para cada uno de los dato del json de entrada
                ...
                celdaoperaciones.setAttribute("identificador",identificador)
                celdaoperaciones.textContent = "🗑️"
                celdaoperaciones.onclick = function(){
                    console.log("has hecho click en una papelera")
                    let idlocal = this.getAttribute("identificador")
                    fetch("eliminaregistro.php?id="+idlocal+"&tabla="+globaltabla)
                    .then(function(response){
                        return response.json()
                    })
                    .then(function(datos){
                        setTimeout(function(){
                            cargaTabla(globaltabla)
                        },1000)
                    })
                }
